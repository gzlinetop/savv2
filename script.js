...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateMediaSession(track, playing=false){
    ...
    try{
      ...
      // Additional handlers (seek/seekback etc)
      try{
        navigator.mediaSession.setActionHandler('seekbackward', (details) => {
          // default 10s when not specified
          ...
          try {
            ...
            if(p && p.getCurrentTime && p.seekTo){
              const t = (p.getCurrentTime() || 0) - seekOffset;
        ...
        // algunos navegadores lanzan si no soportan handlers
      }

      navigator.mediaSession.playbackState = playing ? 'playing' : 'paused';

      // update position state when available
      syncMediaPosition();
  ...
  async function youtubeSearch(query, maxResults=CONFIG.YT_MAX_RESULTS){
    ...
    for(let i=0;i<CONFIG.API_KEYS.length;i++){
      ...
      try{
        ...
        if(!res.ok) {
          // si viene error JSON con información (quota), comprobamos y continuamos a la siguiente key
          try{
            const err = await res.clone().json();
            console.warn('YT API error:', err);
          }catch(e){}
          continue;
        }
